...
typedef struct {
    ...
    i2c_protocol_t i2c;
    zx_device_t* zxdev;
    pdev_vmo_buffer_t mmios[AML_I2C_COUNT];
...
static void aml_i2c_release(void* ctx) {
    aml_i2c_t* i2c = ctx;
    for (unsigned i = 0; i < countof(i2c->mmios); i++) {
        pdev_vmo_buffer_release(&i2c->mmios[i]);
    }
    free(i2c);
}
...
printf("aml_i2c_bind\n");
    ...
    for (unsigned i = 0; i < countof(i2c->mmios); i++) {
        status = pdev_map_mmio_buffer(&i2c->pdev, 0, ZX_CACHE_POLICY_UNCACHED_DEVICE,
                                      &i2c->mmios[i]);
        if (status != ZX_OK) {
            zxlogf(ERROR, "aml_i2c_bind: pdev_map_mmio_buffer failed\n");
            goto fail;
        }
    }
    ...
    i2c->i2c.ops = &i2c_ops;
    i2c->i2c.ctx = i2c;
printf("call pbus_set_protocol ZX_PROTOCOL_I2C\n");
